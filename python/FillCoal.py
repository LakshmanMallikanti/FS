...
def canCompleteCircuit(coal, cost):
    if sum(coal) < sum(cost):
        return -1  # No way to complete the circuit
...
# Input
coal = [int(x) for x in input("Enter coal at stations: ").split()]
cost = [int(x) for x in input("Enter cost to next station: ").split()]
print(canCompleteCircuit(coal, cost))
