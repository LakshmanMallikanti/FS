...
public class HighestVolatility {
    public static int findHighestVolatility(int n, int l, int r, int[] nums) {
        int maxVolatility = -1;
        ...
        for (int size = l; size <= r; size++) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (!minDeque.isEmpty() && minDeque.peekFirst() < i - size + 1)
                    minDeque.pollFirst();
                if (!maxDeque.isEmpty() && maxDeque.peekFirst() < i - size + 1)
                    maxDeque.pollFirst();

                // Maintain minDeque (smallest element at front)
                while (!minDeque.isEmpty() && nums[minDeque.peekLast()] > nums[i]) {
                    minDeque.pollLast();
                }
                minDeque.offerLast(i);

                // Maintain maxDeque (largest element at front)
                while (!maxDeque.isEmpty() && nums[maxDeque.peekLast()] < nums[i]) {
                    maxDeque.pollLast();
                }
                maxDeque.offerLast(i);

                // Calculate volatility for valid windows
                if (i >= size - 1) {
                    int minValue = nums[minDeque.peekFirst()];
                    int maxValue = nums[maxDeque.peekFirst()];
                    maxVolatility = Math.max(maxVolatility, maxValue - minValue);
                }
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(findHighestVolatility(n, l, r, nums));
    }
}
