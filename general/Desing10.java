...
class OrderProcessor implements Runnable {
   ...
   public void run() {
        ...
        String msg = "Order accepted: " + order.getCustomerName() + 
                     " ordered " + order.getQuantity() + " of " + order.getProductName();
        outputMap.put(indexedOrder.index, msg);
} catch (Exception e){
        String msg = "Order failed: " + order.getCustomerName() + " - " + e.getMessage();
        outputMap.put(indexedOrder.index, msg);
}
    }
}